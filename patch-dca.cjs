...
const strings = {
    ...
    "No price data available for this period": { es: "No hay datos de precios para este período", pt: "Sem dados de preços para este período", tr: "Bu dönem için fiyat verisi yok", hi: "इस अवधि के लिए कोई मूल्य डेटा उपलब्ध नहीं है", ru: "Нет данных о ценах за этот период" },
...
["es", "pt", "tr", "hi", "ru"].forEach(lang => {
    ...
    for (const [enKey, translations] of Object.entries(strings)) {
        // Add escaping for single quotes
