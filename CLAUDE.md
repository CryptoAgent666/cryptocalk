...
- Две дыры, обе закрыты в `constants_freshness_monitor.py` (бэкап `.bak-20260829-svgpath`):
  ...
  **Схема поднята `val3→val4`, `txt2→txt3`** — иначе фикс дал бы разовый шквал ложных «changed» на
  ...
  ✅ **Замерено 2026-08-31 (радиус фикса по флоту): 226 из 1049 источников — 21,5%.**
  Замерщик `/tmp/svg_blast_radius.py` на сервере: фетчит страницу ОДИН раз и гоняет **одни и те же
  байты** через старый экстрактор (`.bak-20260829-svgpath`) и новый, поэтому разница — чистый эффект
  фикса, а не «страница изменилась за неделю». Ничего не пишет: 11 state-файлов после прогона
  побайтно равны снапшоту. По доменам: **calk24-de 59,5%** (88/148), calks-uk 25,3%, calk-nz 18,1%,
  calk-ca 11,6%, calk-usa 11,3%, calk-kg 8,9%, calk-au 8,6%, calk-uz 5,4%, cryptocalk 3,3%
  (calk-kz 1/3 — выборка нерепрезентативна, 28 из 31 источников заблокированы).
  Вывод: без бампа схемы `val3→val4`/`txt2→txt3` флот получил бы **226 ложных «changed» за один
  цикл** — это утопило бы настоящие изменения. Бамп был не перестраховкой, а необходимостью.
  ⚠️ Точное деление 226 на «сменились ЦИФРЫ» (`val3→val4`) и «сменился хэш прозы» (`txt2→txt3`)
  НЕ измерялось: в лог попали лишь первые 25 примеров, и они смещены к calk-au/ca/kg (в них 4 val
  против 21 txt). Операционно деление ничего не меняет — обе ветки мигрируют молча.
  ⚠️ 276 источников не сравнились (блокировки/сеть). Это блокировки ПРЯМОГО захода с сервера:
  замерщик не использует render-assist, поэтому реальные слепые зоны монитора меньше.

- 🔴 **PDF-парсер — процессорная яма, из-за неё полный прогон растягивается на часы.**
  Обнаружено 2026-08-31 при попытке прогнать монитор по всему флоту: процесс 21 минуту держал 100%
  ядра (CPU 22:15 при wall 24:33) и закрыл лишь 1 домен из 11. Четыре замера стека `py-spy` подряд
  дали одну точку — `_content_text` (стр. 264) внутри `pdf_text` ← `fetch_hash`. Открытых сокетов
  ноль, то есть это не сеть. Причина: `_content_text` разбирает контент-стримы PDF **побайтно на
  чистом Python** с потолками `max_streams=6000` / `max_chars=3_000_000`, и на толстом документе
  (консолидированный кодекс) молотит минутами. По флоту **51 PDF-источник** (calk-usa 14, calk-nz 13,
  calk-au 11). Диагностика: `sudo -n ~/.local/bin/py-spy dump --pid <PID>` (py-spy ставится
  `python3 -m pip install --user py-spy`; **бинаря `pip3` на сервере нет**, только модуль).
  НЕ ЧИНИЛ: правка общефлотовая, а сменившиеся подписи 51 PDF потребуют ещё одного бампа схемы.
