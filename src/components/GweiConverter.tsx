...
function GweiConverter({ lang = 'en' }: { lang?: string }) {
    ...
    const fetchEthPrice = useCallback(async () => {
        ...
        try {
            const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd&x_cg_demo_api_key=${import.meta.env.PUBLIC_COINGECKO_API_KEY || ''}`);
